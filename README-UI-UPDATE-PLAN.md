...
- [x] Phase 8: release verification and deployment — homepage iframe-height regression repaired and verified at 390px; lint completed with six pre-existing warnings and no errors; typecheck passed; production compilation succeeded; focused Playwright regression check passed; Vercel marked commit `294bbb2` Ready and the live mobile storefront was manually verified on 17 Sep 2026.

Implementation notes: Phase 0 completed without application, database, or deployment changes. Source-document and live-site baseline evidence is recorded in `docs/phase-0-baseline.md`. Phase 1 moved the homepage/listing header into the outer Next.js viewport, added a database-backed `/search?q=` page, and hides embedded duplicate headers before iframe measurement. It has not been deployed yet. Phase 8 release evidence: `294bbb2d6abc7c4740fcbb833517d9e17ecad632` is deployed by Raghava's Vercel project at `https://sleepexcellent-web-app-2-two.vercel.app/`. The iframe-height fix reports settled document dimensions from both approved home documents and remeasures them in the outer storefront shell, preventing the prior one-pixel blank-page race. Existing visual tests use DOM-content readiness rather than waiting indefinitely for third-party image downloads; their screenshot baselines were not automatically updated.
