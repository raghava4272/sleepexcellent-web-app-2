...
const catalogueMenuGroups: CatalogueMenuGroup[] = [
  ...
  { label: "Padding beds", href: "/shop?category=beds", products: [
...
export function Header() {
  ...
  const closeAllMenus = () => { setMenuOpen(false); setActiveCatalogueMenu(null); };
  ...
  return (
    <>
      <header className="storefront-header sticky top-0 z-[80] border-b border-line bg-white/95 shadow-sm backdrop-blur-md">
        ...
        <nav aria-label="Primary navigation" className="hidden border-t border-line lg:block"><ul className="site-container flex items-center gap-6 py-3 text-xs font-semibold uppercase tracking-[0.08em]">{catalogueMenuGroups.map((group) => <li className="group relative" key={group.label} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setActiveCatalogueMenu(null); }} onFocus={() => setActiveCatalogueMenu(group.label)} onMouseEnter={() => setActiveCatalogueMenu(group.label)} onMouseLeave={() => setActiveCatalogueMenu(null)}><button aria-expanded={activeCatalogueMenu === group.label} className="whitespace-nowrap underline-offset-8 hover:underline" onClick={() => setActiveCatalogueMenu((open) => open === group.label ? null : group.label)} type="button">{group.label}</button><div className={`absolute left-0 top-8 z-[100] border border-[#ded9d2] bg-white p-5 normal-case tracking-normal shadow-2xl ${activeCatalogueMenu === group.label ? "block" : "hidden"} group-hover:block ${group.children ? "w-[min(900px,calc(100vw-3rem))] rounded-2xl" : "w-[min(340px,calc(100vw-3rem))] rounded-2xl"}`}><div className={group.children ? "grid grid-cols-3 gap-5" : ""}>{group.children ? group.children.map((child) => <section key={child.label}><Link className="block border-b border-[#d9d1c7] pb-2 text-sm font-bold text-[#5f4531] hover:underline" href={child.href} onClick={closeAllMenus}>{child.label}</Link><ul className="mt-2 space-y-1.5">{child.products.map((item) => <li key={item.slug}><Link className="block text-xs leading-4 text-[#4f5353] hover:text-black hover:underline" href={`/products/${item.slug}`} onClick={closeAllMenus}>{item.label}</Link></li>)}</ul></section>) : <section><Link className="block border-b border-[#d9d1c7] pb-2 text-sm font-bold text-[#5f4531] hover:underline" href={group.href} onClick={closeAllMenus}>View all {group.label}</Link><ul className="mt-2 space-y-1.5">{group.products?.map((item) => <li key={item.slug}><Link className="block text-xs leading-4 text-[#4f5353] hover:text-black hover:underline" href={`/products/${item.slug}`} onClick={closeAllMenus}>{item.label}</Link></li>)}</ul></section>}</div></div></li>)}<li className="ml-auto"><Link className="whitespace-nowrap rounded-full border border-[#8a694c] px-3 py-1.5 normal-case tracking-normal hover:bg-[#f7f5f1]" href="/build-your-mattress">Make your own mattress →</Link></li></ul></nav>
        <div aria-hidden={!menuOpen} className={`border-t border-line bg-white lg:hidden ${menuOpen ? "block" : "hidden"}`} id="mobile-navigation"><nav aria-label="Mobile navigation" className="site-container py-3"><ul className="divide-y divide-line border-y border-line">{catalogueMenuGroups.map((group) => <li key={group.label}><button aria-expanded={activeCatalogueMenu === group.label} className="flex w-full items-center justify-between py-3 text-left text-sm font-semibold" onClick={() => setActiveCatalogueMenu((open) => open === group.label ? null : group.label)} type="button">{group.label}<span aria-hidden="true">{activeCatalogueMenu === group.label ? "−" : "+"}</span></button>{activeCatalogueMenu === group.label ? <div className="pb-3">{group.children ? group.children.map((child) => <section className="border-t border-line py-3" key={child.label}><Link className="text-sm font-bold text-[#5f4531]" href={child.href} onClick={closeAllMenus}>{child.label}</Link><ul className="mt-2 grid gap-1">{child.products.map((item) => <li key={item.slug}><Link className="text-xs text-[#4f5353]" href={`/products/${item.slug}`} onClick={closeAllMenus}>{item.label}</Link></li>)}</ul></section>) : <section className="border-t border-line py-3"><Link className="text-sm font-bold text-[#5f4531]" href={group.href} onClick={closeAllMenus}>View all {group.label}</Link><ul className="mt-2 grid gap-1">{group.products?.map((item) => <li key={item.slug}><Link className="text-xs text-[#4f5353]" href={`/products/${item.slug}`} onClick={closeAllMenus}>{item.label}</Link></li>)}</ul></section>}</div> : null}</li>)}<li><button className="flex w-full items-center justify-between py-3 text-left text-sm font-semibold" onClick={() => openAuth("login")} type="button">Account <span aria-hidden="true">→</span></button></li></ul></nav></div>
      </header>
      {authMode ? <AuthModal error={searchParams.get("error")} message={searchParams.get("message")} mode={authMode} next={currentPath} onClose={closeAuth} onModeChange={changeAuthMode} /> : null}
    </>
  );
}
