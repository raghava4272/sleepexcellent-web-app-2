...
export default async function AdminCategoriesPage() {
  ...
  return <main className="min-h-screen bg-[#f8f4ec] px-5 py-8 text-[#171717] md:px-10"><header className="mx-auto flex max-w-7xl flex-wrap items-end justify-between gap-4 border-b border-[#d6c8b5] pb-6"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#9d6b36]">SleepExcellent admin</p><h1 className="mt-2 font-serif text-4xl">Interior catalogue</h1><p className="mt-2 text-sm text-neutral-600">Control the three customer-facing Interior groups. Pricing is maintained separately because every Interior project is quoted after consultation.</p></div><nav className="flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold"><Link href="/admin">Home</Link><Link href="/admin/categories">Products / Interior</Link><Link href="/interiors">View Interior</Link></nav></header><section className="mx-auto max-w-7xl py-8"><CategoryManager categories={categories} /></section></main>;
}
